...
// Fetch the current user's profile. Inlined here (rather than a
// controller function) since it's a thin wrapper with no reuse elsewhere.
router.get('/me', authenticateUser, async (req, res) => {
  try {
    ...
    console.error(`[users.router] GET /me failed (userId=${req.user?.userId}):`, error.message);
...
// Update a user's loyalty score (admin only).
